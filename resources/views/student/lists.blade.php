@extends('layout2')
@section('body')
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 用户管理 <span class="c-gray en">&gt;</span> 学员列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>	<div class="page-container">
		<div class="text-c">
			<form method="get" action="{{ URL::route('manage.student.lists') }}">
				<input type="text" class="input-text" style="width:250px" placeholder="微信昵称,姓名,手机号,身份证" id="" name="keyword">
				<button type="submit" class="btn btn-success" id="" name=""><i class="Hui-iconfont">&#xe665;</i> 搜学员</button>
			</form>
		</div>
		<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a class="btn btn-primary radius" data-title="添加学员" _href="{{ URL::route('manage.student.add') }}" onclick=article_add('添加资讯','{{ URL::route("manage.student.add") }}') href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加学员</a></span> <span class="r">共有数据：<strong>{{count($lists)}}</strong> 条</span> </div>
		<div class="mt-20">
	...
	<script type="text/javascript">
		...
		function member_sms(obj,id){
			layer.confirm('确认要发送短信吗？',function(index){
				$.ajax({
					type: 'post',
					url: "{{ URL::route('manage.student.sms') }}",
					data: {
						id: id
					},
					dataType: "json",
